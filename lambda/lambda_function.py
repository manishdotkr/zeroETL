...
def lambda_handler(event, context):
    try:
        ...
        if action == 'Create':
            ...
            waitForClusterAvailability(sourceClusterName=sourceClusterName , targetClusterName=targetClusterName, noOfTry=1)
            ...
            print("Zero ETL Integration Created Successfully")
            ...
            sendresponse(event, context, "SUCCESS")
            print("Zero ETL Integration Deleted Successfully")
        else:
...
def getRds(rdsIdentifier):
    ...
    clusterName = [cluster['DBClusterIdentifier'] for cluster in clusters if cluster['DBClusterIdentifier'].startswith(rdsIdentifier)]
    clusterArn = [cluster['DBClusterArn'] for cluster in clusters if cluster['DBClusterIdentifier'].startswith(rdsIdentifier)]

    clusterName = clusterName[0] if len(clusterName) > 0 else None
    clusterArn = clusterArn[0] if len(clusterArn) > 0 else None
    if( not clusterName or not clusterArn):
        raise Exception("RDS cluster not Found")
...
def getRedshiftArn(targetClusterName):
    ...
    if(not targetClusterArn):
        raise Exception("Redshift cluster not Found")
